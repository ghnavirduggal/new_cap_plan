...
export default function DailyIntervalPage() {
  ...
  const groupOptions = useMemo(() => {
    ...
    if (!col) return [];
    return Array.from(new Set(transformData.map((row) => String(row[col] ?? "").trim()).filter(Boolean)));
  }, [transformData, groupLevel]);

  const modelOptions = useMemo(() => {
    if (!transformData.length) return [];
    return Array.from(new Set(transformData.map((row) => String(row.Model ?? "").trim()).filter(Boolean)));
  }, [transformData]);

  const yearOptions = useMemo(() => {
    if (!transformData.length) return [];
    return Array.from(new Set(transformData.map((row) => String(row.Year ?? "").trim()).filter(Boolean)));
  }, [transformData]);

  const monthOptions = useMemo(() => {
    if (!transformData.length) return [];
    return Array.from(new Set(transformData.map((row) => String(row.Month ?? "").trim()).filter(Boolean)));
  }, [transformData]);
  ...
  const handlePushToPlan = async () => {
    ...
    const rows = sourceRows
      .map((row) => {
        const date = normalizeDateValue(row.Date ?? row.date);
        const volume = parseNumber(
          row.Daily_Forecast ??
            row.daily_forecast ??
            row.Interval_Forecast ??
            row.interval_forecast ??
            row.Forecast ??
            row.forecast ??
            row.volume
        );
        if (!date || volume === null) return null;
        const payload: Record<string, any> = { date, volume };
        if (!useDaily) {
          const interval =
            row.Interval ??
            row.interval ??
    ...
    if (!rows.length) {
      notify("warning", "No valid forecast rows to push.");
      return;
    }
    ...
    try {
      ...
      notify("success", `Forecast pushed to plan (${rows.length} rows).`);
    } catch (err: any) {
    ...
    <AppShell crumbs="CAP-CONNECT / Forecast / Daily & Interval">
      <div className="forecast-page forecast-volume-page">
        ...
        {effectiveOriginalData.length ? (
          ...
          <DataTable data={effectiveOriginalData} maxRows={8} />
        </section>
        ) : null}
        ...
        {effectiveIntervalData.length ? (
          ...
          <DataTable data={effectiveIntervalData} maxRows={8} />
        </section>
        ) : null}
        ...
        {transformData.length ? (
          ...
          <DataTable data={transformData} maxRows={10} />
        </section>
        ) : null}
        ...
        {showResults ? (
          ...
          {dailyResult?.results?.daily?.length ? (
            <DataTable data={dailyResult.results.daily} maxRows={12} />
          ) : (
            <div className="forecast-placeholder-bar" />
          )}
        </section>
        ...
        {showResults ? (
          ...
          {dailyResult?.results?.interval?.length ? (
            <DataTable data={dailyResult.results.interval} maxRows={12} />
          ) : hasIntervalSource ? (
            <div className="forecast-placeholder-bar" />
          ) : (
        ...
        {showResults && analysis?.tables?.length
          ? analysis.tables.map((table: any, idx: number) => (
              <section key={`table-${idx}`} className="forecast-section-block">
                ...
                {table.rows?.length ? <DataTable data={table.rows} maxRows={12} /> : <div className="forecast-placeholder-bar" />}
              </section>
            ))
          : null}
