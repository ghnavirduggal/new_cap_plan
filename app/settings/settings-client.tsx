...
export default function SettingsClient() {
  ...
  const [uploadMode] = useState<"append" | "replace">("append");
  ...
  const saveTimeseries = async (key: UploadKey, kind: string) => {
    ...
    try {
      ...
      updateUpload(key, { message: `Saved ${rows.length} rows for ${scopeKey || "global"}.` });
  ...
  const headerNote = "Uploads are saved to the selected scope. Choose scope before uploading.";

  return (
    <>
      <section className="section gauravi">
        <h2>Default Settings - Scope</h2>
