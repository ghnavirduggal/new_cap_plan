...
export default function PlanningClient() {
  ...
  const refreshFormOptions = useCallback(async () => {
    ...
    try {
      ...
      setFormOptions((prev) => ({
        ...prev,
        business_areas: res.business_areas ?? prev.business_areas,
      ...
      setPlanForm((prev) => {
        ...
        if (prev.business_area) {
          if (!next.sub_business_area || !(res.sub_business_areas ?? []).includes(next.sub_business_area)) {
            next.sub_business_area = (res.sub_business_areas ?? [])[0] || "";
          }
          if (!next.location || !(res.locations ?? []).includes(next.location)) {
            next.location = (res.locations ?? [])[0] || "";
          }
          if (!next.site || !(res.sites ?? []).includes(next.site)) {
            next.site = (res.sites ?? [])[0] || "";
          }
        }
        return next;
      });
  ...
  }, [newPlanOpen, notify, planForm.business_area, planForm.location]);
