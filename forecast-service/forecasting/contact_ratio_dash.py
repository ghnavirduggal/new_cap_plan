...
def create_flexible_12_month_test_split(df_smoothed: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, int]:
    ...
    if total_months <= 1:
        return df_smoothed.copy(), df_smoothed.iloc[0:0].copy(), 0
    test_size = int(np.ceil(total_months * 0.3))
    test_size = max(1, min(test_size, total_months - 1))
...
def run_contact_ratio_forecast(
    ...
    if "IQ_value_scaled" not in df_smoothed.columns:
        if "IQ_value" in df_smoothed.columns:
            df_smoothed = _ensure_iq_scaled(df_smoothed)
        else:
            df_smoothed["IQ_value_scaled"] = 0.0
