...
def _rename_columns(df: pd.DataFrame, mapping: dict[str, tuple[str, ...]]) -> pd.DataFrame:
    lookup = {_normalize_col_key(c): c for c in df.columns}
    rename: dict[str, str] = {}
...
def IQ_data(file, font_color: str = "darkblue", font_size: int = 30) -> Dict[str, pd.DataFrame]:
    ...
    df_vol = _rename_columns(
        df_vol,
        {
            "Category": ("Category", "category", "Service_Category_Sub_Service", "Service Category Sub Service"),
            "Date": ("Date", "date"),
    ...
    has_volume_inputs = {"Category", "Date", "Volume"}.issubset(df_vol.columns)
    if not has_volume_inputs:
        warnings.warn("Volume sheet missing required columns for IQ/volume ratios")
    ...
    for cat in grouped_iq["Category"].unique():
        ...
        def fmt_millions(x):
            try:
                return f"{float(x) / 1_000_000:.1f}M" if pd.notna(x) else ""
            except Exception:
                return ""
...
def plot_contact_ratio_seasonality(
    ...
    for i, year in enumerate(melted_ratio_df["Year"].unique()):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(size=6),
                text=[f"{r:.1f}" for r in df_year["Ratio"]],
    ...
    capped_seasonality[month_cols] = capped_seasonality.apply(normalize_row, axis=1).round(1)
    capped_seasonality["Avg"] = capped_seasonality[month_cols].mean(axis=1).round(1)
...
def process_forecast_results(
    ...
    if not combined_forecast_df.empty:
        ...
        cols = ["Model"]
        if "Avg" in wide_forecast_df.columns:
            cols.append("Avg")
        cols.extend(ordered_months)
        cols.extend([c for c in wide_forecast_df.columns if c not in cols])
...
def format_percentage_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df_copy.columns:
        if col != "Year":
            df_copy[col] = df_copy[col].astype(float).round(1)
...
def format_percentage_series(series: pd.Series, col_name: str) -> pd.Series:
    if col_name == "Year":
        return series
    return series.astype(float).round(1).apply(lambda x: f"{x:.1f}%")
...
def prepare_styled_forecast(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.select_dtypes(include="number").columns:
        if col != "Year":
            df[col] = (df[col] * 100).apply(lambda x: f"{x:.1f}%" if pd.notnull(x) else "")
    return df
    ...
    try:
        ...
        for col in columns_to_format:
            if col in ratio_df.columns:
                ratio_df[col] = pd.to_numeric(ratio_df[col], errors="coerce").round(1)
...
def accuracy_phase2_long(
    ...
    for df_summary in (orig_summary, norm_summary):
        ...
        for col in percent_cols:
            df_summary[col] = df_summary[col].map(lambda x: f"{x:.1f}%" if pd.notnull(x) else "")
