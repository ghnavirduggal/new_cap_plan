...
def _di_normalize_distribution(dist: pd.DataFrame) -> pd.DataFrame:
    ...
    if total > 0:
        dist["Distribution_Pct"] = (vals / total * 100).round(1)
    return dist
...
def _di_interval_ratios(interval_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, str]:
    ...
    d["interval"] = d[ivl_col].apply(_normalize_interval_label)
...
def _di_build_forecasts(
    ...
    dist_msg = f"{dist_msg} Normalized (sum {distribution['Distribution_Pct'].sum():.1f}%)."
