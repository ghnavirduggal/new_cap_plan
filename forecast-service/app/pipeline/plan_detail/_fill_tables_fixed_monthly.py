...
def _fill_tables_fixed_monthly(ptype, pid, fw_cols, _tick, whatif=None):
    ...
    calc_bundle = get_cached_consolidated_calcs(int(pid), settings=settings, version_token=_tick, channel=ch_first)
    ...
    try:
        ...
        if ch_low in ("voice", "chat", "outbound", "ob"):
            ...
            def _as_req(df: pd.DataFrame) -> pd.DataFrame:
                if not isinstance(df, pd.DataFrame) or df.empty:
                    return pd.DataFrame()
                out = df.copy()
                # Preserve arrival_load for sumproduct rollups; fallback to volume if present
                if "arrival_load" not in out.columns and "volume" in out.columns:
                    out["arrival_load"] = pd.to_numeric(out.get("volume"), errors="coerce").fillna(0.0)
                if "fte_req" in out.columns and "total_req_fte" not in out.columns:
                    out["total_req_fte"] = pd.to_numeric(out["fte_req"], errors="coerce").fillna(0.0)
                return out
            if ch_low == "voice":
                req_daily_actual = _as_req(calc_bundle.get("voice_day_a", req_daily_actual))
                req_daily_forecast = _as_req(calc_bundle.get("voice_day_f", req_daily_forecast))
                req_daily_tactical = _as_req(calc_bundle.get("voice_day_t", req_daily_tactical))
            elif ch_low == "chat":
                req_daily_actual = _as_req(calc_bundle.get("chat_day_a", req_daily_actual))
                req_daily_forecast = _as_req(calc_bundle.get("chat_day_f", req_daily_forecast))
                req_daily_tactical = _as_req(calc_bundle.get("chat_day_t", req_daily_tactical))
            else:
                req_daily_actual = _as_req(calc_bundle.get("ob_day_a", req_daily_actual))
                req_daily_forecast = _as_req(calc_bundle.get("ob_day_f", req_daily_forecast))
                req_daily_tactical = _as_req(calc_bundle.get("ob_day_t", req_daily_tactical))
    except Exception:
        pass
    ...
    def _daily_to_monthly(df, is_bo: bool):
        ...
        if "arrival_load" in d.columns:
            ...
            d["_fte_x_load"] = d["total_req_fte"] * d["arrival_load"]
            g = d.groupby("month", as_index=False)[["_fte_x_load", "arrival_load"]].sum()
            req_m = {}
            for _, r in g.iterrows():
                load = float(r.get("arrival_load", 0.0) or 0.0)
                mid = str(r["month"])
                req_m[mid] = float(r.get("_fte_x_load", 0.0)) / max(1e-9, load) if load > 0 else 0.0
            return req_m
        g = d.groupby("month", as_index=False)["total_req_fte"].sum()
        return {str(r["month"]): float(r["total_req_fte"]) for _, r in g.iterrows()}
    ...
    def _monthly_hc_step_from_roster(roster_df, mids, role_regex=r"\bagent\b"):
        ...
        first_m = pd.to_datetime(mids_sorted[0], errors="coerce")
        if pd.isna(first_m):
            return {m: 0 for m in mids}
        def _to_ts(val):
            if val is None:
        ...
        for _, r in X.iterrows():
            ...
            psm_ts = _to_ts(psm)
            tm_ts = _to_ts(tm)
            started_before = (psm_ts is None) or (psm_ts < first_m)
            terminated_before_or_on = (tm_ts is not None) and (tm_ts <= first_m)
            if started_before and not terminated_before_or_on:
                base += 1
            if psm is not None and psm in diffs and psm_ts is not None and psm_ts >= first_m:
                diffs[psm] += 1
            if tm is not None and tm in diffs and tm_ts is not None and tm_ts >= first_m:
                diffs[tm] -= 1
        out = {}
        running = base
    ...
    planned_m = _monthly_reduce(planned_df, value_candidates=("hc","headcount","value","count"), how="mean")
    for m in month_ids:
        hc.loc[hc["metric"] == "Budgeted HC (#)",         m] = float(budget_m.get(m, 0.0))
        plan_val = float(planned_m.get(m, 0.0)) if planned_m else 0.0
        if plan_val == 0.0:
            plan_val = float(budget_m.get(m, 0.0))
        hc.loc[hc["metric"] == "Planned/Tactical HC (#)", m] = plan_val
    ...
    try:
        ...
        if isinstance(day_df, pd.DataFrame) and not day_df.empty and "date" in day_df.columns:
            ...
            d = d.dropna(subset=["date"])
            d["month"] = pd.to_datetime(d["date"]).dt.to_period("M").dt.to_timestamp().dt.date.astype(str)
            cap_map = d.groupby("month", as_index=True)["phc"].sum().to_dict() if "phc" in d.columns else {}
            if "arrival_load" in d.columns and "service_level" in d.columns:
                ...
                sl_map = d.groupby("month").apply(_wavg).to_dict()
            else:
                sl_map = d.groupby("month", as_index=True)["service_level"].mean().to_dict() if "service_level" in d.columns else {}
            for mm in month_ids:
                if ch_low in ('voice','chat','outbound','ob'):
                    if mm in cap_map:
    ...
    if ("FTE Required @ Actual Volume" in spec["upper"]) and is_bo_monthly and (bo_model != "erlang"):
        for mm in month_ids:
            ...
            use_actual = (mm_date is not None and mm_date <= cur_month)
            sh_pct = float(shr_actual_pct_m.get(mm, np.nan)) if use_actual else np.nan
            sh_frac = (sh_pct/100.0) if (not pd.isna(sh_pct)) else float(planned_shrink_fraction)
            denom = max(1e-6, monthly_hours * float(util_bo) * max(0.01, 1.0 - sh_frac))
            fte = ((vol * sut) / 3600.0) / denom
    ...
    try:
        ...
        for mm in month_ids:
            ...
            try:
                ...
                mm_date = None
            use_actual = (mm_date is not None and mm_date <= cur_month)
            sh_act_pct = float(shr_actual_pct_m.get(mm, np.nan)) if use_actual else np.nan
