...
def register_plan_detail_core(app: dash.Dash):
        ...
        @app.callback(
            ...
            State("plan-view-range", "data"),
            State("url-router", "pathname"),
            prevent_initial_call=False,
        )
        def _init_cols(pid, grain, interval_date, _refresh_tick, view_range, pathname):
            path = (pathname or "").rstrip("/")
            if not path.startswith("/plan/"):
                raise dash.exceptions.PreventUpdate
            ...
            if isinstance(view_range, dict):
                view_from = view_range.get("from")
                view_to = view_range.get("to")
            if view_from and view_to and str(grain or "week").lower() == "week":
                weeks = _week_span(view_from, view_to)
            else:
        ...
        @app.callback(
            ...
            Output("plan-refresh-tick", "data", allow_duplicate=True),
            Output("plan-view-range", "data", allow_duplicate=True),
        ...
        def _view_apply(_n, dfrom, dto):
            ...
            view_range = {"from": dfrom, "to": dto}
            return (cols,)*9 + (tick, view_range)
