...
from capacity_core import fractional_agents, offered_load_erlangs, service_level, asa, min_agents
from ._common import _assemble_chat, _assemble_ob, _learning_curve_for_week, _load_ts_with_fallback, _scope_key
import json, ast

# Local Erlang helpers (used by interval calc)
...
def _erlang_sl(calls_per_ivl: float, aht_sec: float, agents: float, asa_sec: int, ivl_sec: int) -> float:
    ...
    pw = _erlang_c(A, int(math.ceil(agents)))
    return max(
        0.0,
        min(
            1.0,
            1.0 - pw * math.exp(-max(0.0, (agents - A)) * (asa_sec / max(1.0, aht_sec))),
        ),
    )
from ._common import (
    _canon_scope,
...
_CONSOLIDATED_SLICE_CACHE: dict[tuple[int, int, str, int, str], dict] = {}
...
def invalidate_consolidated_cache(pid: Any) -> None:
    """Drop consolidated_calcs cache entries for a plan (used when uploads/timeseries change)."""
    try:
        pid_int = int(pid)
    except Exception:
        return
    keys = [k for k in _CONSOLIDATED_CACHE.keys() if int(k[0]) == pid_int]
    for k in keys:
        _CONSOLIDATED_CACHE.pop(k, None)
    slice_keys = [k for k in _CONSOLIDATED_SLICE_CACHE.keys() if int(k[0]) == pid_int]
    for k in slice_keys:
        _CONSOLIDATED_SLICE_CACHE.pop(k, None)
...
def get_cached_consolidated_calcs(
    ...
    *,
    ...
    lob_first = (plan.get("channel") or plan.get("lob") or "").split(",")[0].strip()
    site_scope = (plan.get("site") or plan.get("location") or plan.get("country") or "").strip()
    ...
    ch_key = str(channel or "all").strip().lower()
    if ch_key and ch_key != "all":
        cached_slice = _CONSOLIDATED_SLICE_CACHE.get((*key, ch_key))
        if cached_slice is not None:
            return cached_slice
    cached = _CONSOLIDATED_CACHE.get(key)
    if cached is None:
        cached = consolidated_calcs(
            pid,
            "week",
            plan_date=plan_date,
        ...
        _CONSOLIDATED_CACHE[key] = cached
    if not ch_key or ch_key == "all":
        return cached
    if ch_key in {"voice", "inbound"}:
        prefixes = ("voice_",)
    elif ch_key in {"chat"}:
        prefixes = ("chat_",)
    elif ch_key in {"outbound", "ob"}:
        prefixes = ("ob_",)
    elif ch_key in {"back office", "backoffice", "bo"}:
        prefixes = ("bo_",)
    else:
        prefixes = ()
    if not prefixes:
        return cached
    sliced = {k: v for k, v in (cached or {}).items() if isinstance(k, str) and k.startswith(prefixes)}
    if len(_CONSOLIDATED_SLICE_CACHE) > 64:
        _CONSOLIDATED_SLICE_CACHE.pop(next(iter(_CONSOLIDATED_SLICE_CACHE)), None)
    _CONSOLIDATED_SLICE_CACHE[(*key, ch_key)] = sliced
    return sliced
...
def _weekly_hc_step_from_roster(roster: pd.DataFrame, week_ids: list[str], role_regex: str) -> dict[str, int]:
    ...
    first_week = pd.to_datetime(week_ids[0], errors="coerce")
    if pd.isna(first_week):
        return {w: 0 for w in week_ids}
    def _to_ts(val):
        if val is None:
            return None
    ...
    for _, r in X.iterrows():
        ...
        psw_ts = _to_ts(psw)
        tw_ts = _to_ts(tw)
        started_before = (psw_ts is None) or (psw_ts < first_week)
        terminated_before_or_on = (tw_ts is not None) and (tw_ts <= first_week)
        if started_before and not terminated_before_or_on:
            base += 1
        if psw is not None and psw in diffs and psw_ts is not None and psw_ts >= first_week:
            diffs[psw] += 1
        if tw is not None and tw in diffs and tw_ts is not None and tw_ts >= first_week:
            diffs[tw] -= 1
...
def _voice_interval_calc(ivl_df: pd.DataFrame, settings: dict, ivl_min: int) -> pd.DataFrame:
    ...
    ivl_sec = max(60.0, float(ivl_sec))

    def _call_capacity_per_hour(agents: float, aht_sec: float) -> float:
        agents_int = int(math.floor(agents)) if agents > 0 else 0
        if agents_int <= 0 or aht_sec <= 0:
            return 0.0
        max_calls = int(math.ceil((3600.0 / float(aht_sec)) * float(agents_int)))
        lo, hi = 0, max_calls
        while lo < hi:
            mid = (lo + hi + 1) // 2
            req_agents = fractional_agents(target_sl, T_sec, float(mid), aht_sec)
            if req_agents <= float(agents_int):
                lo = mid
            else:
                hi = mid - 1
        return float(lo)
...
def _weekly_from_daily(day_df: pd.DataFrame, week_start: str = "Monday") -> pd.DataFrame:
    ...
    for (w,p), grp in df.groupby(["week","program"], as_index=False):
        ...
        fte = _weighted_avg(fte_vals, loads) if sum(loads) > 0 else float(sum(fte_vals))
...
def consolidated_calcs(
    ...
    """
    plan = get_plan(pid) or {}
    ch_first = (plan.get("channel") or plan.get("lob") or "").split(",")[0].strip()
    site_scope = (plan.get("site") or plan.get("location") or plan.get("country") or "").strip()
    settings = dict(
        settings
        or resolve_settings(
    ...
    def _has_intervals(df: pd.DataFrame) -> bool:
        if not _chat_any(df) or "interval" not in df.columns:
            return False
        try:
            s = df["interval"]
            # Treat empty strings as missing
            s = s.replace("", pd.NA) if hasattr(s, "replace") else s
            return bool(pd.Series(s).notna().any())
        except Exception:
            return False
    def _is_ivl(df):
        return _has_intervals(df)
...
def _fill_tables_fixed(ptype, pid, fw_cols, _tick, whatif=None, grain: str = 'week'):
    ...
    for w in week_ids:
        try:
            settings_by_week[w] = resolve_settings(
                ba=p.get("vertical"),
                subba=p.get("sub_ba"),
                lob=ch_first,
                site=(p.get("site") or p.get("location") or p.get("country") or "").strip(),
            ...
            settings_by_week[w] = settings

    def _settings_for_week(w):
        return settings_by_week.get(w, settings)

    def _pct_from_setting(val, default_pct):
        try:
            if isinstance(val, str) and val.strip().endswith("%"):
                return float(val.strip()[:-1])
            out = float(val)
            return out * 100.0 if out <= 1.0 else out
        except Exception:
            return float(default_pct)

    def _frac_from_setting(val, default_frac):
        try:
            out = float(val)
            return out / 100.0 if out > 1.0 else out
        except Exception:
    ...
    calc_bundle = get_cached_consolidated_calcs(
        ...
        plan_date=None,
        version_token=_tick,
        channel=(p.get("channel") or p.get("lob") or "").split(",")[0].strip().lower(),
    )
    ...
    occ_w = {}
    for w in week_ids:
        sw = _settings_for_week(w)
        if ch_key in ("voice",):
            occ_base_raw = sw.get("occupancy_cap_voice", sw.get("occupancy", 0.85))
        elif ch_key in ("back office", "bo"):
            occ_base_raw = sw.get("util_bo", 0.85)
        elif ch_key in ("outbound",):
            occ_base_raw = sw.get("util_ob", 0.85)
        elif ch_key in ("chat",):
            occ_base_raw = sw.get("util_chat", sw.get("util_bo", 0.85))
        else:
            occ_base_raw = _setting(sw, ["occupancy","occupancy_pct","target_occupancy","budgeted_occupancy","occ","occupancy_cap_voice"], 0.85)
        occ_w[w] = int(round(_pct_from_setting(occ_base_raw, 85.0)))
    ...
    def _weekly_chat(df: pd.DataFrame):
        ...
        if "interval" in d.columns:
            daily_rows = []
            for day, grp in d.groupby("date", as_index=False):
                v = pd.to_numeric(grp.get("items"), errors="coerce").fillna(0.0)
                n = pd.to_numeric(grp.get("_num"), errors="coerce").fillna(0.0)
                if len(v) > 1 and (v.nunique(dropna=True) == 1):
                    daily_rows.append({"date": day, "items": float(v.mean()), "_num": float(n.mean())})
                else:
                    daily_rows.append({"date": day, "items": float(v.sum()), "_num": float(n.sum())})
            daily = pd.DataFrame(daily_rows)
            daily["week"] = (pd.to_datetime(daily["date"]) - pd.to_timedelta(pd.to_datetime(daily["date"]).dt.weekday, unit="D")).dt.date.astype(str)
            vol = daily.groupby("week", as_index=True)["items"].sum()
            num = daily.groupby("week", as_index=True)["_num"].sum()
        else:
    ...
    def _weekly_ob(df: pd.DataFrame):
        ...
        if "interval" in d.columns:
            daily_rows = []
            for day, grp in d.groupby("date", as_index=False):
                v = pd.to_numeric(grp.get("_exp"), errors="coerce").fillna(0.0)
                n = pd.to_numeric(grp.get("_num"), errors="coerce").fillna(0.0)
                if len(v) > 1 and (v.nunique(dropna=True) == 1):
                    daily_rows.append({"date": day, "_exp": float(v.mean()), "_num": float(n.mean())})
                else:
                    daily_rows.append({"date": day, "_exp": float(v.sum()), "_num": float(n.sum())})
            daily = pd.DataFrame(daily_rows)
            daily["week"] = (pd.to_datetime(daily["date"]) - pd.to_timedelta(pd.to_datetime(daily["date"]).dt.weekday, unit="D")).dt.date.astype(str)
            vol = daily.groupby("week", as_index=True)["_exp"].sum()
            num = daily.groupby("week", as_index=True)["_num"].sum()
        else:
    ...
    def _attach_arrival_load(req_df: pd.DataFrame, src_df: pd.DataFrame) -> pd.DataFrame:
        if not isinstance(req_df, pd.DataFrame) or req_df.empty:
            return req_df
        if "arrival_load" in req_df.columns:
            return req_df
        if not isinstance(src_df, pd.DataFrame) or src_df.empty:
            return req_df
        cols = {c.lower(): c for c in src_df.columns}
        date_col = cols.get("date")
        vol_col = cols.get("volume") or cols.get("calls") or cols.get("items")
        if not date_col or not vol_col:
            return req_df
        try:
            base = src_df[[date_col, vol_col]].copy()
            base["date"] = pd.to_datetime(base[date_col], errors="coerce").dt.date
            base["arrival_load"] = pd.to_numeric(base[vol_col], errors="coerce").fillna(0.0)
            base = base.dropna(subset=["date"])
            base = base.groupby("date", as_index=False)["arrival_load"].sum()
            out = req_df.copy()
            out["date"] = pd.to_datetime(out["date"], errors="coerce").dt.date
            out = out.merge(base, on="date", how="left")
            out["arrival_load"] = pd.to_numeric(out.get("arrival_load"), errors="coerce").fillna(0.0)
            return out
        except Exception:
            return req_df

    try:
        req_daily_tactical = _attach_arrival_load(req_daily_tactical, vT)
        req_daily_budgeted = _attach_arrival_load(req_daily_budgeted, vB)
        req_daily_forecast = _attach_arrival_load(req_daily_forecast, vF)
        req_daily_actual = _attach_arrival_load(req_daily_actual, vA)
    except Exception:
        pass
    ...
    def _daily_to_weekly(df, ch_first=ch_first, settings=settings):
        ...
        if "arrival_load" in d.columns:
            g = d.groupby("week", as_index=False).agg(
                ...
                arrival_load=("arrival_load", "sum")
            )
            out = {}
            for _, r in g.iterrows():
                load = float(r.get("arrival_load", 0.0) or 0.0)
                if load > 0:
                    # Weighted average (sumproduct): sum(fte * load) / sum(load)
                    w = d[d["week"] == r["week"]].copy()
                    f = pd.to_numeric(w["total_req_fte"], errors="coerce").fillna(0.0)
                    l = pd.to_numeric(w["arrival_load"], errors="coerce").fillna(0.0)
                    out[str(r["week"])] = float((f * l).sum() / max(1e-9, l.sum()))
                else:
                    out[str(r["week"])] = float(r.get("total_req_fte", 0.0))
            return out
        g = d.groupby("week", as_index=False)["total_req_fte"].sum()
        return g.set_index("week")["total_req_fte"].to_dict()
    ...
    planned_nh_w = {
        w: int(round(planned_nh_w.get(w, 0) * max(0.0, min(1.0,( (_lc_with_wf(_learning_curve_for_week(_settings_for_week(w), lc_ovr_df, w), w).get("throughput_train_pct", 100.0) / 100.0) *
              (_lc_with_wf(_learning_curve_for_week(_settings_for_week(w), lc_ovr_df, w), w).get("throughput_nest_pct", 100.0) / 100.0) )
        ))))
        for w in week_ids
    }
    ...
    budget_df = _first_non_empty_ts(sk, ["hc_budget","budget_headcount","budget_hc","headcount_budget","hc_budget"])
    planned_df = _first_non_empty_ts(sk, ["hc_planned","headcount_planned","planned_headcount","hc_plan"])
    budget_w  = _weekly_reduce(budget_df, value_candidates=("hc","headcount","value","count"), how="sum")
    planned_w = _weekly_reduce(planned_df, value_candidates=("hc","headcount","value","count"), how="sum")
    for w in week_ids:
        hc.loc[hc["metric"] == "Budgeted HC (#)",         w] = float(budget_w.get(w, 0.0))
        plan_val = float(planned_w.get(w, 0.0)) if planned_w else 0.0
        if plan_val == 0.0:
            plan_val = float(budget_w.get(w, 0.0))
        hc.loc[hc["metric"] == "Planned/Tactical HC (#)", w] = plan_val
    ...
    saved_plan_pct_w = {}
    use_saved_shr = bool(settings.get("use_saved_shrinkage"))
    if use_saved_shr:
        try:
    ...
    try:
        ...
        if ch_low in ("voice","chat","outbound","ob"):
            res = calc_bundle or {}
            if ch_low == "voice":
                day_a = res.get("voice_day_a")
                day_f = res.get("voice_day_f")
                day_t = res.get("voice_day_t")
            elif ch_low == "chat":
                day_a = res.get("chat_day_a")
                day_f = res.get("chat_day_f")
                day_t = res.get("chat_day_t")
            else:
                day_a = res.get("ob_day_a")
                day_f = res.get("ob_day_f")
                day_t = res.get("ob_day_t")
            if isinstance(day_f, pd.DataFrame) and not day_f.empty:
                w = _weekly_from_daily(day_f)
                for _, r in w.iterrows():
                    req_w_forecast[str(pd.to_datetime(r["week"]).date())] = float(r.get("fte_req", 0.0))
            if isinstance(day_a, pd.DataFrame) and not day_a.empty:
                w = _weekly_from_daily(day_a)
                for _, r in w.iterrows():
                    req_w_actual[str(pd.to_datetime(r["week"]).date())] = float(r.get("fte_req", 0.0))
            if isinstance(day_t, pd.DataFrame) and not day_t.empty:
                w = _weekly_from_daily(day_t)
                for _, r in w.iterrows():
                    req_w_tactical[str(pd.to_datetime(r["week"]).date())] = float(r.get("fte_req", 0.0))
    except Exception:
        pass
    ...
    for w in week_ids:
        ...
        sw = _settings_for_week(w)
        bo_model = (sw.get("bo_capacity_model") or "tat").lower()
        bo_wd = int(sw.get("bo_workdays_per_week", 5))
        bo_hpd = float(sw.get("bo_hours_per_day", sw.get("hours_per_fte", 8.0)))
        bo_shr_base = _frac_from_setting(sw.get("bo_shrinkage_pct", sw.get("shrinkage_pct", 0.0)), 0.0)
        voice_shr_base = _frac_from_setting(sw.get("voice_shrinkage_pct", sw.get("shrinkage_pct", 0.0)), 0.0)
        util_bo = float(sw.get("util_bo", 0.85) or 0.85)
        if ch_low == "voice":
            agents_prod = float(schedule_supply_avg.get(w, projected_supply.get(w, 0.0)))
            lc = _lc_with_wf(_learning_curve_for_week(sw, lc_ovr_df, w), w)
            ...
            try:
                ...
                ot = 0.0
            if ot:
                wd_voice = int(sw.get("workdays_per_week", 7) or 7)
                hpd      = float(sw.get("hours_per_fte", 8.0) or 8.0)
            ...
            lc = _learning_curve_for_week(sw, lc_ovr_df, w)
            ...
            ob_util = float(sw.get("util_ob", sw.get("occupancy_cap_voice", 0.85)) or 0.85)
            lc = _learning_curve_for_week(sw, lc_ovr_df, w)
            ...
            hrs = float(sw.get("hours_per_fte", 8.0) or 8.0)
            cov_min = int(float(sw.get("ob_coverage_minutes", hrs * 60.0) or (hrs * 60.0)))
            ivl_per_week = int(max(1, round((cov_min / ivl_min_ob)))) * max(1, bo_wd)
            handling_capacity[w] = items_per_ivl * ivl_per_week
        elif "chat" in ch_low:
            # Chat via Erlang with concurrency
            chat_util = float(sw.get("util_chat", sw.get("util_bo", 0.85)) or 0.85)
            conc = float(sw.get("chat_concurrency", 1.0) or 1.0)
            ...
            hrs = float(sw.get("hours_per_fte", 8.0) or 8.0)
            cov_min = int(float(sw.get("chat_coverage_minutes", hrs * 60.0) or (hrs * 60.0)))
    ...
    for w in week_ids:
        ...
        util_bo = float(sw.get("util_bo", 0.85) or 0.85)
        if ch_low == "voice":
            weekly_load = float(weekly_demand_voice.get(w, 0.0)) # type: ignore
            ...
            if (agents_prod is None or agents_prod <= 0) and (float(req_w_forecast.get(w, 0.0) or 0.0) > 0.0):
                try:
                    hrs_per_fte = float(sw.get("hours_per_fte", 8.0) or 8.0)
                except Exception:
                    hrs_per_fte = 8.0
                try:
                ...
                staff_sec_per_day = float(req_w_forecast.get(w, 0.0) or 0.0) * hrs_per_fte * 3600.0 * max(0.01, occ_f)
                agents_eff_fb = staff_sec_per_day / float(max(1, daily_intervals) * max(60, ivl_sec))
                agents_prod = agents_eff_fb
            agents_eff_avg = max(1.0, (float(agents_prod) + nest_eff + sda_eff))
    ...
    try:
        ch_low = str(ch_first or "").strip().lower()
        res = calc_bundle or {}
        # Prefer interval->daily->weekly sums for PHC when available
        day_key = None
        if ch_low == "voice":
            day_key = "voice_day_f" if res.get("voice_day_f") is not None else ("voice_day_a" if res.get("voice_day_a") is not None else "voice_day_t")
        elif ch_low == "chat":
            day_key = "chat_day_f" if res.get("chat_day_f") is not None else ("chat_day_a" if res.get("chat_day_a") is not None else "chat_day_t")
        elif ch_low in ("outbound", "ob"):
            day_key = "ob_day_f" if res.get("ob_day_f") is not None else ("ob_day_a" if res.get("ob_day_a") is not None else "ob_day_t")
        day_df = res.get(day_key, pd.DataFrame()) if day_key else pd.DataFrame()
        if isinstance(day_df, pd.DataFrame) and not day_df.empty and "date" in day_df.columns:
            d = day_df.copy()
            d["date"] = pd.to_datetime(d["date"], errors="coerce").dt.date
            d = d.dropna(subset=["date"])
            d["week"] = d["date"].apply(lambda x: (x - dt.timedelta(days=x.weekday())).isoformat())
            cap_map = d.groupby("week", as_index=True)["phc"].sum().to_dict() if "phc" in d.columns else {}
            if "arrival_load" in d.columns and "service_level" in d.columns:
                def _wavg(grp):
                    l = pd.to_numeric(grp["arrival_load"], errors="coerce").fillna(0.0)
                    s = pd.to_numeric(grp["service_level"], errors="coerce").fillna(0.0)
                    return float((s * l).sum() / max(1e-9, l.sum())) if float(l.sum()) > 0 else float(s.mean())
                # sl_map = d.groupby("week").apply(_wavg).to_dict()
                sl_map = d.groupby("week").apply(_wavg, include_groups=False).to_dict()
            else:
                sl_map = d.groupby("week", as_index=True)["service_level"].mean().to_dict() if "service_level" in d.columns else {}
            for wid in week_ids:
                if ch_low in ("voice","chat","outbound","ob"):
                    if wid in cap_map:
    ...
    try:
        ...
        for w in week_ids:
            ...
            if s_plan is None:
                s_plan = planned_shrink_fraction
            s_act = _to_frac_local(act_row.get(w)) if isinstance(act_row, dict) and (w in act_row) else None
            try:
                w_date = pd.to_datetime(w, errors="coerce").date()
            except Exception:
                w_date = None
            ...
            # Actual tooltip (actual shrink for past/current, else planned)
            s_used = s_plan
