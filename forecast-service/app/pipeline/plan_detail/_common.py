...
from app.pipeline.planning_store import upsert_plan as _upsert_plan, load_plan as _load_plan
...
_PLANS_INDEX_KEY = "plans_index"  # legacy; metadata now lives in planning_plans
...
def _load_index() -> pd.DataFrame:
    ...
    return pd.DataFrame({"plan_id": pd.Series(dtype="int64")})


def _parse_hierarchy(value) -> dict:
...
def get_plan_meta(pid: int | str) -> dict:
    ...
    try:
        ...
        return {}
    plan = get_plan(pid_i) or {}
    if not plan:
        return {}
    hier = _parse_hierarchy(plan.get("hierarchy_json"))
    meta = dict(hier or {})
    meta.update(
        {
            "plan_id": pid_i,
            ...
            "org": plan.get("org"),
            "business_entity": plan.get("business_entity"),
            "business_area": plan.get("business_area") or plan.get("vertical"),
            "sub_business_area": plan.get("sub_business_area") or plan.get("sub_ba"),
            "vertical": plan.get("vertical") or plan.get("business_area"),
            "sub_ba": plan.get("sub_ba") or plan.get("sub_business_area"),
            "channel": plan.get("channel"),
            "location": plan.get("location"),
            "site": plan.get("site"),
            "start_week": plan.get("start_week"),
            "end_week": plan.get("end_week"),
            "ft_weekly_hours": plan.get("ft_weekly_hours"),
            "pt_weekly_hours": plan.get("pt_weekly_hours"),
            "status": plan.get("status"),
            "is_current": plan.get("is_current"),
            "tags": plan.get("tags"),
            "owner": plan.get("owner"),
            "created_by": plan.get("created_by"),
            "created_on": plan.get("created_at") or plan.get("created_on"),
            "last_updated_by": plan.get("updated_by") or plan.get("last_updated_by"),
            "last_updated_on": plan.get("updated_at") or plan.get("last_updated_on"),
        }
    )
    return meta

def save_plan_meta(pid: int, meta: dict) -> None:
    """Upsert plan metadata into planning_plans + hierarchy_json."""
    pid = int(pid)
    meta = dict(meta or {})
    base = _load_plan(pid) or {}
    if not base:
        return
    ...
    field_map = {
        ...
        "vertical": "business_area",
        "sub_business_area": "sub_business_area",
        "sub_ba": "sub_business_area",
        "channel": "channel",
        "location": "location",
        "site": "site",
        "start_week": "start_week",
        "end_week": "end_week",
        "ft_weekly_hours": "ft_weekly_hours",
        "pt_weekly_hours": "pt_weekly_hours",
        "status": "status",
    ...
    for key, target in field_map.items():
        if key in meta and meta[key] is not None:
            payload[target] = meta[key]

    if meta.get("created_by"):
        payload["created_by"] = meta.get("created_by")
    updated_by = meta.get("last_updated_by") or meta.get("updated_by")
    if updated_by:
        payload["updated_by"] = updated_by

    hier = _parse_hierarchy(payload.get("hierarchy_json"))
    if isinstance(meta.get("hierarchy_json"), dict):
        hier.update(meta.get("hierarchy_json") or {})

    reserved = set(field_map.keys()) | {
        "plan_id",
        "created_on",
        "created_by",
        "last_updated_on",
    ...
    payload["hierarchy_json"] = sanitize_for_json(hier)
    _upsert_plan(payload)
    try:
        from .calc_engine import mark_plan_dirty_deps
        mark_plan_dirty_deps(pid, "plan_meta")
    except Exception:
        try:
            mark_plan_dirty(pid)
        except Exception:
...
def _settings_for_scope_key(sk: str) -> dict:
    try:
        ba, sba, ch, site = (sk.split("|") + ["", "", "", ""])[:4]
    except Exception:
        ba, sba, ch, site = "", "", "", ""
    return resolve_settings(ba=ba, subba=sba, lob=ch, site=site)
...
def _weekly_voice(df):
    ...
    def _daily_from_intervals(frame: pd.DataFrame, vol_col: str, aht_col: str) -> pd.DataFrame:
        if frame.empty:
            return pd.DataFrame(columns=["date", "vol", "num"])
        out_rows = []
        has_ivl = "interval" in frame.columns
        for d, grp in frame.groupby("date", as_index=False):
            v = pd.to_numeric(grp.get(vol_col), errors="coerce").fillna(0.0)
            a = pd.to_numeric(grp.get(aht_col), errors="coerce").fillna(0.0)
            num = v * a
            if has_ivl and len(v) > 1 and (v.nunique(dropna=True) == 1):
                out_rows.append({"date": d, "vol": float(v.mean()), "num": float(num.mean())})
            else:
                out_rows.append({"date": d, "vol": float(v.sum()), "num": float(num.sum())})
    ...
    g = daily.groupby("week", as_index=False).agg(vol=("vol", "sum"), num=("num", "sum"))
...
def _weekly_bo(df):
    ...
    def _daily_from_intervals(frame: pd.DataFrame, items_col: str, sut_col: str) -> pd.DataFrame:
        if frame.empty:
            return pd.DataFrame(columns=["date", "items", "num"])
        out_rows = []
        has_ivl = "interval" in frame.columns
        for d, grp in frame.groupby("date", as_index=False):
            v = pd.to_numeric(grp.get(items_col), errors="coerce").fillna(0.0)
            a = pd.to_numeric(grp.get(sut_col), errors="coerce").fillna(0.0)
            num = v * a
            if has_ivl and len(v) > 1 and (v.nunique(dropna=True) == 1):
                out_rows.append({"date": d, "items": float(v.mean()), "num": float(num.mean())})
            else:
                out_rows.append({"date": d, "items": float(v.sum()), "num": float(num.sum())})
        return pd.DataFrame(out_rows)
...
def _assemble_voice(scope_key, which):
    ...
    # Fallback: some uploads only persist combined datasets (voice_{which}) with AHT + volume included.
    combo = pd.DataFrame()
    used_combo_for_vol = False
    if (vol is None or vol.empty) or (aht is None or aht.empty):
        for cand in (f"voice_{which}", f"voice_{which}_aht_volume"):
            try:
    ...
    if isinstance(aht, pd.DataFrame) and not aht.empty:
        ...
        # Coalesce when volume already carried aht_sec (e.g. combined uploads)
        ...
        # Backward safety: handle older merge suffixes if present
...
def _dedupe_timeseries(df: pd.DataFrame) -> pd.DataFrame:
    if not isinstance(df, pd.DataFrame) or df.empty:
        return df
    if "date" not in df.columns:
        return df
    if "interval" in df.columns:
        interval_norm = df["interval"].astype(str).str.strip()
        def _canon_ivl(x: str) -> str:
            ...
            try:
                ...
                pass
            # Fallback for HH:MM(:SS) without date
            ...
            return s
        interval_norm = interval_norm.map(_canon_ivl)
        has_ivl = interval_norm.replace("nan", "").replace("NaT", "").ne("").any()
        if has_ivl:
            out = df.copy()
            out["_interval_norm"] = interval_norm
            out = out.drop_duplicates(subset=["date", "_interval_norm"], keep="last")
            return out.drop(columns=["_interval_norm"])
...
def _first_non_empty_ts(scope_key: str, keys: list[str]) -> pd.DataFrame:
    ...
    for k in keys:
        ...
        if isinstance(df, pd.DataFrame) and not df.empty:
            return df.copy()
    def _is_attr_pct_key(k: str) -> bool:
        kl = str(k).lower()
        return ("attrition" in kl) and ("pct" in kl or "percent" in kl or "rate" in kl)
    if any(_is_attr_pct_key(str(k)) for k in keys):
        df = _attrition_weekly_fallback(scope_key)
        if isinstance(df, pd.DataFrame) and not df.empty:
            return df.copy()
    return pd.DataFrame()

def _attrition_weekly_fallback(scope_key: str) -> pd.DataFrame:
    try:
    ...
    if "program" in df.columns and ba:
        prog = df["program"].astype(str).str.strip().str.lower()
        df = df[prog.isin({ba, "all", "overall", "global"})].copy()
    if df.empty:
        return pd.DataFrame()
    if "attrition_pct" not in df.columns and {"leavers_fte", "avg_active_fte"}.issubset(df.columns):
        denom = df["avg_active_fte"].replace({0: np.nan})
        df = df.copy()
        df["attrition_pct"] = (df["leavers_fte"] / denom) * 100.0
    if "week" in df.columns and "date" not in df.columns:
...
def _weekly_reduce(df: pd.DataFrame, value_candidates=("value","hc","headcount","hours","items","volume","count","amt","amount"),
    ...
    if not isinstance(df, pd.DataFrame) or df.empty:
        return {}
    date_col = None
    for c in ("date","dt","day","when","week","week_start","week_start_date"):
        if c in df.columns:
            date_col = c
            break
    if not date_col:
