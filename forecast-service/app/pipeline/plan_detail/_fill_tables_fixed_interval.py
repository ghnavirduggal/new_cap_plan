...
def _actual_shrink_for_week(pid: int, week_key: str) -> float | None:
    try:
        shr = load_df(f"plan_{pid}_shr")
        if not isinstance(shr, pd.DataFrame) or shr.empty or "metric" not in shr.columns:
            return None
        m = shr["metric"].astype(str).str.strip().str.lower()
        row = shr.loc[m == "overall shrinkage %"]
        if row.empty:
            return None
        val = row.iloc[0].to_dict().get(week_key)
        return _to_frac(val)
    except Exception:
        return None
...
def _slot_series_for_day(df: pd.DataFrame, day: dt.date, val_col: str) -> Dict[str, float]:
    """Map df for a single day into slot (HH:MM) -> value without aggregation.
    If multiple rows share the same slot, the last non-null value wins.
    """
    base = _prepare_day_slot_frame(df, day)
    if not isinstance(base, pd.DataFrame) or base.empty or val_col not in base.columns:
        return {}
    vals = pd.to_numeric(base[val_col], errors="coerce")
    tmp = pd.DataFrame({"lab": base["lab"], "val": vals}).dropna(subset=["lab"])
    if tmp.empty:
        return {}
    out: Dict[str, float] = {}
...
def _erlang_sl(A_calls: float, aht: float, agents: float, ivl_sec: float, T_sec: float) -> float:
    ...
    pw = _erlang_c(A, int(max(1, math.ceil(agents))))
    return max(
        0.0,
        min(
            1.0,
...
def _min_agents_cached(calls_rounded: int, aht_rounded: int,
    ...
    """Cached wrapper for Erlang fractional agents; occ_cap_scaled kept for cache key stability."""
    calls = float(calls_rounded)
    aht = float(aht_rounded)
    ivl_min = max(1, int(ivl_min))
    ...
    agents = fractional_agents(float(target_sl), float(T_sec), calls_per_hour, aht)
    sl = service_level(traffic, int(math.ceil(agents)) if agents > 0 else 0, aht, float(T_sec))
...
@lru_cache(maxsize=100_000)
def _call_capacity_per_hour_cached(
    agents_int: int, target_sl: float, T_sec: int, aht_rounded: int
) -> float:
    """Max calls/hour supported by agents_int at target SL/ASA (Excel CallCapacity)."""
    if agents_int <= 0 or aht_rounded <= 0:
        return 0.0
    max_calls = int(math.ceil((3600.0 / float(aht_rounded)) * float(agents_int)))
    lo, hi = 0, max_calls
    while lo < hi:
        mid = (lo + hi + 1) // 2
        req_agents = fractional_agents(float(target_sl), float(T_sec), float(mid), float(aht_rounded))
...
def _fill_tables_fixed_interval(
    ...
    if ch == "voice":
        ...
        today = dt.date.today()
        try:
            ref_date = pd.to_datetime(ref_day).date()
        except Exception:
            ref_date = None
        shrink = _planned_shrink(settings, ch)
        if ref_date is not None and ref_date <= today:
            wk = _week_start(ref_date).isoformat()
            s_act = _actual_shrink_for_week(pid, wk)
        ...
        for lab in ivl_ids:
            ...
            # Projected handling capacity (CallCapacity) + SL
            cap = 0.0
            sl_val = 0.0
            if aht_round > 0 and ivl_sec > 0:
                agents_for_cap = base_nf if base_nf > 0 else (base_na if base_na > 0 else ag)
                agents_int = int(math.floor(agents_for_cap)) if agents_for_cap > 0 else 0
                if agents_int > 0:
                    calls_per_hour = _call_capacity_per_hour_cached(
                        agents_int, float(target_sl), int(T_sec), int(aht_round)
                    )
                    # Display capacity per hour (DASH behavior)
                    cap = float(math.floor(float(calls_per_hour / 2)))
                    # SL uses calls-per-hour input (Excel SLA expects calls per hour)
                    calls_per_hour_sl = float(calls)
                    calls_round_for_sl = int(round(calls_per_hour_sl))
                    traffic_hr = (calls_per_hour_sl * float(aht_round)) / ivl_sec if aht_round > 0 else 0.0
                    sl_val = 100.0 * service_level(
                        float(traffic_hr),
                        int(math.ceil(agents_for_cap)) if agents_for_cap > 0 else 0,
                        float(aht_round),
                        float(T_sec),
                    )
