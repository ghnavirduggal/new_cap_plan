...
from app.pipeline.capacity_core import min_agents
...
def _voice_interval_calc(ivl_df: pd.DataFrame, settings: dict, ivl_min: int) -> pd.DataFrame:
    ...
    occ_cap = settings.get("occupancy_cap", settings.get("occupancy_cap_voice", 0.85))

    rows = []
    for _, row in df.iterrows():
        calls = _safe_float(row.get("volume"), 0.0)
        aht = _safe_float(row.get("aht_sec"), 0.0)
        agents, sl, occ, _asa = min_agents(calls, aht, int(ivl_sec / 60), target_sl, target_sec, occ_cap)
...
def consolidated_calcs(
    ...
    *,
    ...
    chat_ivl_f = _chat_interval_calc(chat_f, settings, ivl_min) if _has_intervals(chat_f) else pd.DataFrame()
    chat_ivl_a = _chat_interval_calc(chat_a, settings, ivl_min) if _has_intervals(chat_a) else pd.DataFrame()
    chat_ivl_t = _chat_interval_calc(chat_t, settings, ivl_min) if _has_intervals(chat_t) else pd.DataFrame()
    res["chat_ivl_f"], res["chat_ivl_a"], res["chat_ivl_t"] = chat_ivl_f, chat_ivl_a, chat_ivl_t
    res["chat_day_f"] = _daily_from_intervals(chat_ivl_f, settings, "items") if not chat_ivl_f.empty else _bo_daily_calc(chat_f, settings)
    res["chat_day_a"] = _daily_from_intervals(chat_ivl_a, settings, "items") if not chat_ivl_a.empty else _bo_daily_calc(chat_a, settings)
    res["chat_day_t"] = _daily_from_intervals(chat_ivl_t, settings, "items") if not chat_ivl_t.empty else _bo_daily_calc(chat_t, settings)
    ...
    ob_ivl_f = _ob_interval_calc(ob_f, settings, ivl_min) if _has_intervals(ob_f) else pd.DataFrame()
    ob_ivl_a = _ob_interval_calc(ob_a, settings, ivl_min) if _has_intervals(ob_a) else pd.DataFrame()
    ob_ivl_t = _ob_interval_calc(ob_t, settings, ivl_min) if _has_intervals(ob_t) else pd.DataFrame()
    res["ob_ivl_f"], res["ob_ivl_a"], res["ob_ivl_t"] = ob_ivl_f, ob_ivl_a, ob_ivl_t
    res["ob_day_f"] = _daily_from_intervals(ob_ivl_f, settings, "items") if not ob_ivl_f.empty else _bo_daily_calc(ob_f, settings)
    res["ob_day_a"] = _daily_from_intervals(ob_ivl_a, settings, "items") if not ob_ivl_a.empty else _bo_daily_calc(ob_a, settings)
    res["ob_day_t"] = _daily_from_intervals(ob_ivl_t, settings, "items") if not ob_ivl_t.empty else _bo_daily_calc(ob_t, settings)
    res["ob_ivl"] = ob_ivl_f
    ...
    res["bo_day_f"] = _bo_daily_calc(bo_f, settings) if not bo_f.empty else pd.DataFrame()
    res["bo_day_a"] = _bo_daily_calc(bo_a, settings) if not bo_a.empty else pd.DataFrame()
    res["bo_day_t"] = _bo_daily_calc(bo_t, settings) if not bo_t.empty else pd.DataFrame()
