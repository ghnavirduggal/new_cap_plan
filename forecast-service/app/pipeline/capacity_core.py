...
def voice_requirements_interval(voice_df: pd.DataFrame, settings: dict) -> pd.DataFrame:
    ...
    ivl_sec = out["interval"].map(lambda val: _ivl_minutes_from_str(val, ivl_min_default)).fillna(ivl_min_default).astype(int) * 60

    rows = []
    for idx, row in out.iterrows():
        calls = float(row["calls"] or 0.0)
        aht = float(row["aht_sec"] or 0.0)
        ivl_seconds = int(ivl_sec.iloc[idx])
        ivl_min = max(5, round(ivl_seconds / 60))
        traffic = offered_load_erlangs(calls, aht, ivl_min)
        agents, sl, occ, avg_asa = min_agents(calls, aht, ivl_min, target_sl, target_sec, occ_cap)
        rows.append(
            {
                "date": row["date"],
                ...
                "occupancy": occ,
                "asa_sec": avg_asa,
                "staff_seconds": agents * ivl_seconds,
            }
        )
...
def bo_rollups(bo_df: pd.DataFrame, settings: dict, week_start: str = "Monday") -> dict:
    ...
    denom = hours * 3600.0 * (1.0 - shrink) * util
...
def bo_erlang_rollups(bo_df: pd.DataFrame, settings: dict, week_start: str = "Monday") -> dict:
    ...
    for _, row in grouped.iterrows():
        ...
        rows.append({"date": row["date"], "program": row["program"], "fte_req": agents})
    daily = pd.DataFrame(rows)

    denom = hours * 3600.0 * max(1e-6, (1.0 - shrink)) * occ_cap
...
def required_fte_daily(voice_df: pd.DataFrame, bo_df: pd.DataFrame, ob_df: pd.DataFrame, settings: dict) -> pd.DataFrame:
    ...
    if isinstance(ob_df, pd.DataFrame) and not ob_df.empty:
        ...
        def _staff_seconds(row: pd.Series) -> float:
            ...
            if calls <= 0:
                return 0.0
            aht = float(row.get("_aht", 0.0) or 0.0)
            if aht <= 0:
                return 0.0
            ...
            if not ivl_min or ivl_min <= 0:
                ivl_min = coverage_min
            ivl_min = max(1.0, ivl_min)
            agents, _sl, _occ, _asa = min_agents(calls, aht, int(round(ivl_min)), target_sl_ob, target_sec_ob, occ_cap_ob)
            return agents * ivl_min * 60.0
