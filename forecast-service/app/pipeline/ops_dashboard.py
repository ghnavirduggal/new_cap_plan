...
def _compute_ops_base(
    ...
    if not voice.empty and "volume" in voice.columns:
        v_w = voice.groupby("date", as_index=False)["volume"].sum().rename(columns={"volume": "weight"})
        weight_df = v_w
    if not bo.empty and "items" in bo.columns:
        b_w = bo.groupby("date", as_index=False)["items"].sum().rename(columns={"items": "weight"})
        if weight_df.empty:
            weight_df = b_w
        else:
            weight_df = weight_df.merge(b_w, on="date", how="outer", suffixes=("_v", "_b"))
            weight_df["weight"] = weight_df[["weight_v", "weight_b"]].fillna(0.0).sum(axis=1)
            weight_df = weight_df[["date", "weight"]]
    t_weight = time.perf_counter()

    logger.info(
        "ops_dashboard base timings scopes=%.3fs voice=%.3fs bo=%.3fs settings=%.3fs req=%.3fs roster=%.3fs hiring=%.3fs supply=%.3fs weight=%.3fs total=%.3fs scopes=%s voice_rows=%s bo_rows=%s roster_rows=%s hiring_rows=%s",
        (t_scopes - t0),
        (t_voice - t_scopes),
        (t_bo - t_voice),
        (t_settings - t_bo),
...
def refresh_ops(
    ...
    if not voice.empty and "volume" in voice.columns:
        v_w = (
            voice.groupby("date", as_index=False)["volume"]
            .sum()
    ...
    if not bo.empty and "items" in bo.columns:
        b_w = (
            bo.groupby("date", as_index=False)["items"]
            .sum()
            .rename(columns={"items": "weight"})
        )
        if weight_df.empty:
            weight_df = b_w
        else:
    ...
    if not gap_df.empty:
        ...
        if not peak_req_row.empty:
            peak_required = {
                "date": str(peak_req_row.iloc[0]["bucket"]),
                "value": float(peak_req_row.iloc[0]["total_req_fte"]),
            }
        if not peak_sup_row.empty:
            peak_supply = {
                "date": str(peak_sup_row.iloc[0]["bucket"]),
                "value": float(peak_sup_row.iloc[0]["supply_fte"]),
            }
        if not worst.empty:
            worst_gap = {
                "date": str(worst.iloc[0]["bucket"]),
                "value": float(worst.iloc[0]["gap_fte"]),
            }
        if not best.empty:
            best_gap = {
                "date": str(best.iloc[0]["bucket"]),
                "value": float(best.iloc[0]["gap_fte"]),
            }
    else:
        peak_required = {}
        peak_supply = {}
    ...
    if grain == "interval" and not voice.empty and "interval" in voice.columns and voice["interval"].notna().any():
        vi = voice_requirements_interval(voice, settings)
        if not vi.empty:
            # Aggregate across scopes/programs to avoid duplicate timestamps when multiple queues are present.
            if "date" in vi.columns and "interval" in vi.columns:
                vi = (
                    vi.groupby(["date", "interval"], as_index=False)["agents_req"]
                    .sum()
    ...
    logger.info(
        "ops_dashboard timings scope=%.3fs load=%.3fs calc=%.3fs supply=%.3fs render=%.3fs total=%.3fs scopes=%s voice_rows=%s bo_rows=%s",
        (t_scopes - t0),
        (t_load - t_scopes),
        (t_calc - t_load),
        (t_supply - t_calc),
