...
const sections: Section[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Segment (Plan Detail options): an optional free-text dimension on a plan (e.g. tenure, language, LOB, customer tier). Set it on the plan, then filter the Planning list by segment. It is a tag for organising plans and does not affect any calculation.",
      "Custom dimensions (Planning workspace → Manage dimensions): define your own ordered dimensions (e.g. tenure band, language) with optional allowed-value lists. Each plan can then be tagged with values for those dimensions (Plan Detail → Dimensions), and the Planning list can filter by any dimension and value. Like Segment, dimensions organise plans and do not affect any calculation or rollup.",
      "Demand by Dimension (Plan Detail options and BA rollup view): an opt-in view that re-buckets the per-scope FTE balance (required / supply / shortfall / coverage) by a custom dimension. Tag scopes with dimension values in the Planning workspace (Scope tags), via the ingest API (a 'dimensions' object), or the scope-dimensions endpoint; the grouping only re-aggregates already-computed numbers, so it never changes a staffing figure, and the default BA/SBA/Channel/Site rollup is untouched.",
      "Approval workflow (Plan Detail options): a plan moves Draft → Submitted → Approved or Rejected (with Reopen back to Draft). Each transition is attributed (who + when + optional note) and logged to the activity feed as an audit trail. When per-plan authorization is enabled, only admins can approve or reject.",
