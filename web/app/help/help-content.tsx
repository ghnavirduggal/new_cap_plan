...
const sections: Section[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Demand by Dimension (Plan Detail options): an opt-in view that re-buckets the per-scope FTE balance (required / supply / shortfall) by a custom dimension. Scopes are tagged with dimension values via the ingest API (a 'dimensions' object) or the scope-dimensions endpoint; the grouping only re-aggregates already-computed numbers, so it never changes a staffing figure, and the default BA/SBA/Channel/Site rollup is untouched.",
